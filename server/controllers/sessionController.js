...
export const createSession = async (req, res) => {
    try{
        const token = req.body.token || req.headers.authorization?.split(" ")[1]; //extracting uid from request body/ authorization header
...
export const logoutSession = async (req, res) => {
    try{
        ...
        const sessionToken = req.body.sessionToken || req.headers.authorization?.split(" ")[1] 
        if (!sessionToken) {
