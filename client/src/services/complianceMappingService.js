...
import { COMPLIANCE_MAPPING_API } from "./api";
...
export const getBusinessTypesForDropdown = async () => {
  const response = await api.get(`${BUSINESS_TYPES_API}`, { params: { status: "active", limit: 100 } });
  return response.data.data;
};

// Helper to fetch compliance items for dropdown
export const getComplianceItemsForDropdown = async () => {
  const response = await api.get(`${COMPLIANCE_MAPPING_API}`, { params: { status: "active", limit: 100 } });
  return response.data.data;
};